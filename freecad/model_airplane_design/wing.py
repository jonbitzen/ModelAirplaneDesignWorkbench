...
class Wing():
    def __init__(
        ...
        # TODO: here lets generate a couple of hard-coded spars;  the front
        #       spar will be generated between rib 0 and n-1, the rear spar
        #       will be generated between rib 0 and rib n.  We're going to
        #       get the root rib bbox, and locate the front spar 20% back from
        #       the LE, and the rear spar 67% back from the LE.  The front spar
        #       will have a 4x6 profile, and the rear spar will have a 4x5
        #       profile.  Each spar profile will be located at the rib mid-point
        #       Once we've located it, we can make a start and end profile at
        #       each desired rib location, and create a cubic solid.  Once we've
        #       made our cubic solid, we can walk each of the ribs, make a temp
        #       plane from the airfoil outline, and get intersections between
        #       the temp planes and the spar solid.  For now lets add these to
        #       the rib - they are a natural part of the rib, as they will be
        #       pockets in the final solid
        #       Once we've walked each rib and calculated the spar interference
        #       for each, we can generate the lightening holes

        # So here's a thing.  Ribs in the long run may not be aligned to the
        # axis planes, which means that we need a more general notion how to get
        # the distance from the LE and TE.  So, I think what we do is rotate each
        # rib into the xy plane, then use that to find our spar positions.  Then
        # we rotate everything back into the original orientation using the rib's
        # stored transform
        # I think its possible we might eventually want to do this on temp copies,
        # so we dont disturb any geometries generated by the actual wing piece.
        # But for now we'll just work with the original.
        r0 = rib_list[0]
        r1 = rib_list[num_sections-2]
        r2 = rib_list[num_sections-1]

        p0_r = self.__get_spar_point(r0.airfoil, 0.2, WingEdge.LEADING)
        p0_r = Part.show(p0_r)
        p0_r.ViewObject.PointColor = utilities.BLUE(1.0)
        p0_r.ViewObject.PointSize = 10.0

        p0_t = self.__get_spar_point(r1.airfoil, 0.2, WingEdge.LEADING)
        p0_t = Part.show(p0_t)
        p0_t.ViewObject.PointColor = utilities.BLUE(1.0)
        p0_t.ViewObject.PointSize = 10.0

        p1_r = self.__get_spar_point(r0.airfoil, 0.33, WingEdge.TRAILING)
        p1_r = Part.show(p1_r)
